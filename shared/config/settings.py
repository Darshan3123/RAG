...
# Each module has its own .env — only the variables that
# module needs should be set there. Unknown vars are ignored.
# =========================================================
import os
from pathlib import Path
from dotenv import load_dotenv

# Load .env from the directory that contains THIS settings.py's
# grandparent (i.e. the module root, not the shared root).
# Each module (scraper/, indexer/, query/) has its own .env.
# Fallback: walk up until we find a .env file.
def _find_env() -> Path:
...
# ---------------------------------------------------------
# SCRAPER TARGETS  (scraper module)
# ---------------------------------------------------------
...
# Legacy SQLite paths (kept for backward compat / reindex from old DB)
...
# ---------------------------------------------------------
# MONGODB  (shared by scraper + indexer)
# ---------------------------------------------------------
MONGO_URI        = _str("MONGO_URI",        "mongodb://localhost:27017")
MONGO_DB_NAME    = _str("MONGO_DB_NAME",    "gem_scraper")
MONGO_BIDS_COLL  = _str("MONGO_BIDS_COLL",  "bids")
MONGO_RUNS_COLL  = _str("MONGO_RUNS_COLL",  "run_log")
MONGO_TENDER_COLL= _str("MONGO_TENDER_COLL","tenders")

# ---------------------------------------------------------
# TESSERACT  (scraper module — Windows path ignored on Linux)
# ---------------------------------------------------------
TESSERACT_CMD = _str(
    "TESSERACT_CMD",
...
# ---------------------------------------------------------
# RAG SETTINGS  (indexer + query modules)
# ---------------------------------------------------------
...
RAG_FETCH_K       = _int("RAG_FETCH_K", 40)
